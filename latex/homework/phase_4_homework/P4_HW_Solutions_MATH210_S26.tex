...
for parameter values $k>0$, $m>0$, and $M>0$ and an initial value $y_0$.  Your file should plot the solution to the equation with at least three solutions on the same graph, one with $y_0<m$, $m<y_0<M$, and one with $y_0>M$.  In the space below, include the Python figure with a caption.  In the caption, you must say which equilibrium states are attracting and which are repelling.  \\


\begin{figure}[h!]
\begin{center}
\includegraphics[width = .8\textwidth]{p4hw_figure_1.eps}
\end{center}
\caption{A series of trajectories are shown with starting population values that span three different regimes: $0<y_0<m$, $m<y_0<M$, and $y_0>M$.  Here, we can see that the equilibrium solutions $y^* = 0$ and $y^* = M$ are attracting, while the equilibrium state $y^* = m$ is repelling.  }
\label{graph_1}
\end{figure}
...
We can rewrite the differential equation system in the following form: 

\begin{align*}
\de{x}{t} & = Rx\left(1 - \frac{x}{R/4}\right) - 3xy\\ 
\de{y}{t} & = -2y + xy. \end{align*}
Written this way, we can see that the $x$ population experiences logistic growth to the carrying capacity $R/4$ in the absence of the $y$ population.  The $y$ population experiences exponential decay in the absence of $x$.  Therefore, it makes sense that the $x$ population is the prey since the interaction term is negative causing a decay if $y$ is nonzero.  This makes $y$ the predator since the interaction term is positive and it only grows when $x$ is nonzero. \\

If coexistence occurs, we see that a nonzero equilibrium of $x^* = 2$ satisfies the second equation.  Plugging this value into the first equation yields 
\[ y^* = \frac{R - 8}{3},\]
which means $R>8$ for the $y$ population to have a positive equilibrium value.  Figure \ref{graph_2} shows the phase plane and a time series plot for $R = 60$.  As $t\to \infty$, $(x,y) \to (2, 52/3)$, which means both populations approach a positive equilibrium value, i.e.~they coexist.  We note that $x$ does not achieve its carrying capacity due to the presence of the predator population. 

\begin{figure}[h!]
\begin{center}
\includegraphics[width = .48\textwidth]{p4hw_figure_3.eps}
\includegraphics[width = .48\textwidth]{p4hw_figure_2.eps}
\end{center}
\caption{(Left) Phase plane plot with a trajectory and vector field.  (Right) Time-series plot for the same trajectory that shows both populations.  }
\label{graph_2}
\end{figure}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width = .8\textwidth]{p4hw_figure_4.eps}
\end{center}
\caption{A time series plot of each peptide component.  As time $t\to \infty$, larger peptide components eventually break down to smaller and smaller components until they all are identified as $P_4$.  Therefore, $P_4 \to 1$ and $P. P_1, P_2, P_3 \to 0$. }
\label{graph_3}
\end{figure}
