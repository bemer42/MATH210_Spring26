...
def dYdt(t, Y):
    ...
    r1 = np.sqrt((x-xp_1)**2 + (y-yp_1)**2 + (z-zp_1)**2)
    r2 = np.sqrt((x-xp_2)**2 + (y-yp_2)**2 + (z-zp_2)**2)
    r3 = np.sqrt((x-xp_3)**2 + (y-yp_3)**2 + (z-zp_3)**2)
    
    # Define the derivatives: 
    dxdt = vx
    dydt = vy
    dzdt = vz
    dvxdt = -G*M1*(x-xp_1)/r1**3-G*M2*(x-xp_2)/r2**3-G*M3*(x-xp_3)/r3**3
    dvydt = -G*M1*(y-yp_1)/r1**3-G*M2*(y-yp_2)/r2**3-G*M3*(y-yp_3)/r3**3
    dvzdt = -G*M1*(z-zp_1)/r1**3-G*M2*(z-zp_2)/r2**3-G*M3*(z-zp_3)/r3**3
...
plt.plot(xp_1, yp_1, zp_1, 'bo',markersize = 20)
plt.plot(xp_2, yp_2, zp_2, 'mo',markersize = 20)
plt.plot(xp_3, yp_3, zp_3, 'ko',markersize = 20)
plt.plot(x[0], y[0],z[0], 'go', markersize = 3)
...
body, = ax2.plot([], [], [], 'go',markersize=4)
tail, = ax2.plot([], [], [], 'g-', linewidth=1)  
...
# ani.save('One_Body_Movie.mov', writer='ffmpeg', fps=10)
writer = FFMpegWriter(fps=30, codec='h264')
ani.save('One_Body_Movie.mp4', writer=writer)
